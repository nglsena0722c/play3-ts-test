...
const Item = ({ position, tap, useritem }: { position?: string, tap?: Tap, useritem? : UserItem }) => {
    return <div className="relative flex justify-center items-center aspect-square">
        {
            useritem &&
            <div className="z-40 absolute top-0 w-full h-full">
            <Draggable >
                {/* <img src='/play3-ts-test/img/Sunglasses.svg' /> */}
                <div className="w-full h-full hover:cursor-pointer" style={{ backgroundImage: `url("${useritem.imagePath}")`}} />
            </Draggable>
        </div>
        }
